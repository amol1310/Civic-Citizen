...
app.post('/api/auth/send-otp', async (req, res) => {
  ...
  if (twilioClient) {
    try {
      ...
      return res.status(500).json({ error: 'Failed to send real OTP' });
    }
  } else {
...
const handleImageUpload = (file) => {
  return new Promise((resolve, reject) => {
    if (process.env.CLOUDINARY_API_KEY && process.env.CLOUDINARY_API_KEY !== 'mock_key') {
      const uploadStream = cloudinary.uploader.upload_stream({ folder: 'citizenconnect' }, (error, result) => {
...
app.post('/api/complaints', upload.array('images', 3), async (req, res) => {
  try {
    ...
    const complaint = await Complaint.create({
      id: legacyId, user_id, issue_type, description,
...
app.get('/api/complaints', adminAuth, async (req, res) => {
  try {
    ...
    res.json(complaints.map(c => ({ ...c._doc, user_name: c.user_id?.name, mobile_number: c.user_id?.mobile })));
...
app.get('/api/complaints/:id', async (req, res) => {
  try {
    ...
    res.json({ ...complaint._doc, user_name: complaint.user_id?.name, mobile_number: complaint.user_id?.mobile, feedback });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.get('/api/users/my-complaints', async (req, res) => {
  try {
    const complaints = await Complaint.find({ user_id: req.query.user_id }).sort({ created_at: -1 });
    res.json(complaints);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.patch('/api/complaints/:id', adminAuth, async (req, res) => {
  try {
    ...
    if (status && status !== complaint.status) {
      complaint.history.push({ type: 'status_change', from: complaint.status, to: status, updated_by: req.admin.name });
      complaint.status = status;
    }
    if (assignedTo && assignedTo !== complaint.assignedTo) {
      complaint.history.push({ type: 'assignment', from: complaint.assignedTo, to: assignedTo, updated_by: req.admin.name });
...
app.get('/api/admin/stats', adminAuth, async (req, res) => {
  try {
    ...
    const byTypeData = await Complaint.aggregate([{ $group: { _id: "$issue_type", count: { $sum: 1 } } }]);
    const byType = {};
    byTypeData.forEach(item => byType[item._id] = item.count);
    res.json({ total, pending: total - resolved, resolved, byType });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
